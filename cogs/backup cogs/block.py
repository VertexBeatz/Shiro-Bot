...
class StickyRoles:
    ...
    @checks.admin()
    async def setblock(self, ctx, *, role: discord.Role):
        """Write the name of the block role :D
        
        [p]setblock @Blocked"""
        m =ctx.message

        self.db[m.server.id]["block"] = role.id
        self.db[m.server.id]["sticky_roles"].append(role.id)
        self.save()
    ...
    async def jail(self, ctx, *, user: discord.Member=None):
        ...
        if not user:
            await self.bot.say("Tag the person to jail")
            return

        m = ctx.message
        settings = self.db[m.server.id]

        role = discord.utils.get(m.server.roles, id=settings["block"])
        try:
            await self.bot.add_roles(user, role)
            await self.bot.say("{} is now in jail,{}".format(user.name, role.mention))
