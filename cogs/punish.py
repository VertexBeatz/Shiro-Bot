...
class Punish:
    ...
    def __init__(self, bot):
        ...
        self.handles = {}
        self.channelpunish = dataIO.load_json("data/punish/channel.json")
    ...
    @commands.command(pass_context=True, no_pm=True)
    @checks.mod_or_permissions(manage_messages=True)
    async def punishlog(self,ctx, channel: discord.Channel):
        """Set the channel lag."""
        if not channel:
            await self.bot.say("You need to tag a channel or put the channel ID")

        self.channelpunish[ctx.message.server.id] = channel.id
    ...
    async def punish(self, ctx, user: discord.Member, duration: str=None, *, reason: str=None):
        ...
        try:
            async for message in self.bot.logs_from(m.channel, limit=500):
                if message.author == user:
                    await self.bot.delete_message(message)
                    a += 1
                    if self.channelpunish[m.server.id]:
                        yolo = message.server.get_channel(self.channelpunish[m.server.id])
                        await self.bot.send_message(yolo, "Author: {}, Author ID: {} Message deleted: \n {}".format(user.name, user.id, message.content))
                if a >= 12:
                    return
        except:
            pass
